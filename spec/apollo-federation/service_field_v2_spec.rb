  ...
  shared_examples 'service field' do
    ...
    it 'returns the federation SDL for the schema' do
      ...
      expect(execute_sdl(schema)).to match_sdl(
        <<~GRAPHQL,
          extend schema
            @link(url: "https://specs.apollo.dev/federation/v2.6", import: ["@composeDirective", "@inaccessible", "@policy", "@tag"])
    ...
    context 'with a custom link namespace provided' do
      it 'returns valid SDL for type extensions with custom namespace' do
        ...
        expect(execute_sdl(schema)).to match_sdl(
          <<~GRAPHQL,
            extend schema
              @link(url: "https://specs.apollo.dev/federation/v2.6", as: "fed2", import: ["@composeDirective", "@inaccessible", "@policy", "@tag"])
      ...
      it 'returns valid SDL for @interfaceObject directives with custom namespace' do
        ...
        expect(execute_sdl(schema)).to match_sdl(
          <<~GRAPHQL,
            extend schema
              @link(url: "https://specs.apollo.dev/federation/v2.3", as: "fed2", import: ["@composeDirective", "@inaccessible", "@policy", "@tag"])
